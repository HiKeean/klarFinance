...
async function decryptTokenCookie(value: string): Promise<string> {
  try {
    return await decryptAes(value);
  } catch {
    // Kompatibilitas dengan cookie token versi sebelumnya yang memakai 6 bagian.
    const parts = value.split('.');
    if (parts.length !== 6) throw new Error('Cookie token tidak valid');
    return (
      await Promise.all([
...
export class AuthStateService {
  ...
  public readonly userProfile$: Observable<any> = this.userProfileSubject.asObservable();
  ...
  signIn(email: string, password: string) {
    if (!this.auth) return Promise.reject(new Error('Firebase Auth is browser-only.'));
    return signInWithEmailAndPassword(this.auth, email, password);
  }

  async saveSession(accessToken: string, refreshToken: string, userProfile: any, menuList: MenuItem[]): Promise<void> {
    ...
    try {
      ...
      // Simpan satu token terenkripsi utuh. Memecah hasil enkripsi dengan "."
      // membuat token mudah rusak ketika dibaca kembali dari cookie.
  ...
  async getAccessToken(): Promise<string | null> {
    ...
    try {
      ...
      return await decryptTokenCookie(encrypted);
    } catch {
      return null;
    }
  }
  ...
  private scheduleSessionExpiry(token: string): void {
    ...
    } catch { /* Guard/interceptor tetap menangani token invalid. */ }
  }
